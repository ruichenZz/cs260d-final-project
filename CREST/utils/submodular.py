...
def faciliy_location_order(
    ...
    # Cap the budget at the effective ground set size
    budget = min(num_per_class, N - 1)
    if budget == 0:
    ...
    start = time.time()
    print(f"Class {c}: Budget={budget}, Ground Set Size={N}")
    greedyList = obj.maximize(
        budget=budget,
        optimizer="LazyGreedy",
        stopIfZeroGain=False,
...
def get_orders_and_weights(
    ...
    if equal_num:
        ...
        minority = np.array(minority, dtype=bool)
        if sum(minority) > 0:
            extra = sum([max(0, np.ceil(B / C) - class_nums[c]) for c in classes])
            print(f"classes: {classes}, type: {type(classes)}, shape: {classes.shape if hasattr(classes, 'shape') else 'N/A'}")
            print(f"minority: {minority}, type: {type(minority)}, shape: {minority.shape if hasattr(minority, 'shape') else 'N/A'}")
